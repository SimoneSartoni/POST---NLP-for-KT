...
from Knowledge_Tracing.code.experiments.modules.count_vect import *
...
class evaluate_count_vectorizer(experiment):
    def __init__(self):
        super().__init__("evaluate_count_vectorizer", "encoding evaluation")
    ...
    def run(self, current_experiment):
        with start_run(experiment_id=current_experiment.experiment_id, run_name=current_experiment.name):
            ...
            for input_dataset in input_datasets:
                experiments.append([])
                experiments[i].append(count_vect(input_dataset, prediction_model=cosine_similarity_threshold(), load=False))
                experiments[i].append(count_vect(input_dataset, prediction_model=logistic_regressor(), load=False))
