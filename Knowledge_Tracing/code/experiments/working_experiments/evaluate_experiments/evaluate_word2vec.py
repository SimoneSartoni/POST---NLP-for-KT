    ...
    def run(self, current_experiment):
        with start_run(experiment_id=current_experiment.experiment_id, run_name=current_experiment.name):
            ...
            for input_dataset in input_datasets:
                experiments.append([])
                experiments[i].append(word2vec(input_dataset, prediction_model=cosine_similarity_threshold(), load=False))
                experiments[i].append(word2vec(input_dataset, prediction_model=logistic_regressor(), load=False))
