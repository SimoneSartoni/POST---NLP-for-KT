...
from Knowledge_Tracing.code.experiments.modules.tf_idf import tf_idf
from Knowledge_Tracing.code.experiments.modules.tf_idf_for_optimization import *
...
class optimize_tf_idf(experiment):
    def __init__(self):
        super().__init__("optimize_tf_idf", "optimization")
        ...
        with start_run(experiment_id=current_experiment.experiment_id, run_name=current_experiment.name):
            ...
            for input_dataset in input_datasets:
                ...
                for trials in range(0, 10):
                    x = tf_idf(dataset=input_dataset, prediction_model=logistic_regressor(), load=False)
                    ...
                    x.set_params(**{"min_df": min_df_el, "max_df": max_df_el, "topK": topK_el, "shrink": shrink_el,
