...
class perceptron(predictor):
    ...
    def train(self, encoding_model, train_set, validation_set):
        ...
        similarities, labels = generate_similarity_scores(encoding_model, train_set.problems, train_set.labels, train_set.lengths)
        self.prediction_model.fit(similarities, labels)
        print(self.prediction_model.get_params())

    def compute_predictions(self, test_set):
        t = time()
        similarities, self.labels = generate_similarity_scores(self.encoding_model, test_set.problems, test_set.labels, test_set.lengths)
