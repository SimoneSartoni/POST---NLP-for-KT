    ...
    def fit_on_CA(self, texts_df, save_filepath='/content/', text_column="sentence"):
        ...
        for epoch in range(epochs):
            ...
            for batch in loop:
                ...
                anchor_ids = batch['anchor_ids'][0].to(device)
                anchor_mask = batch['anchor_mask'][0].to(device)
                pos_ids = batch['positive_ids'][0].to(device)
                pos_mask = batch['positive_mask'][0].to(device)
                print(anchor_ids)
