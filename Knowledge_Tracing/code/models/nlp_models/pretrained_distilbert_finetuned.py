...
class SentenceSimilarityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        anchor_ids = torch.from_numpy(np.concatenate(anchor_ids)).float()
        anchor_mask = torch.from_numpy(np.concatenate(anchor_mask)).float()
        positive_ids, positive_mask = self.tokenizer(texts_2, padding='max_length', truncation=True)
        positive_ids = torch.from_numpy(np.concatenate(positive_ids)).float()
        positive_mask = torch.from_numpy(np.concatenate(positive_mask)).float()
