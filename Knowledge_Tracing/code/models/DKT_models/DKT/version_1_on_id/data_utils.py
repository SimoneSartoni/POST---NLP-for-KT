...
def create_dataset(generator, ids_depth, shuffle=True, batch_size=1024):
    input_types = {"feature_id": tf.float32, "target_feature_id": tf.float32}
    output_types = {"target_label": tf.float32}

    input_shapes = {"feature_id": [None, ids_depth], "target_feature_id": [None, ids_depth]}
    ...
    dataset = dataset.map(
        lambda inputs, outputs: (
            {"input_feature_id": inputs['feature_id'],
             "target_feature_id": inputs['target_feature_id']},
...
def load_dataset(batch_size=32, shuffle=True,
    ...
    inputs = {"question_id": False, "text_id": False, "skill": False, "feature_id": True,
              ...
              "target_feature": False, "target_feature_id": True}
    ...
    val_loader = create_dataset(val_gen, ids_depth, shuffle=shuffle, batch_size=batch_size)
    test_loader = create_dataset(test_gen, ids_depth, shuffle=shuffle, batch_size=batch_size)
