...
class SAINT(nn.Module):
    def __init__(self, n_encoder, n_decoder, enc_heads, dec_heads, n_dims, total_ex, total_cat, total_responses,
        ...
        self.encoder = get_clones(EncoderBlock(enc_heads, n_dims, total_ex, total_cat, seq_len), n_encoder)
        self.decoder = get_clones(DecoderBlock(dec_heads, n_dims, total_responses, seq_len), n_decoder)
        self.fc = nn.Linear(n_dims, 1)

    def forward(self, inputs, target):
        first_block = True
        in_exercise, in_category, in_response = inputs['input_id'], inputs['input_skill'], inputs['input_label']
        for n in range(self.n_encoder):
            if n >= 1:
                first_block = False

            enc = self.encoder[n](in_exercise, in_category, first_block=first_block)
            in_exercise = enc
            in_category = enc
