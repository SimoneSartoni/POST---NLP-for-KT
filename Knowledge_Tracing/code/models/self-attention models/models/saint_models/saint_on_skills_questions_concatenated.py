...
class saint_on_skills_questions_concatenated(nn.Module):
    ...
    def forward(self, inputs, decoder_targets):
        ...
        in_encoder, in_decoder = self.embedding(in_exercise, in_skill, in_response)
...
class EmbeddingBlock(nn.Module):
    def __init__(self, n_dims, nb_questions, nb_skills, nb_responses,  seq_len):
        ...
        self.response_embed = nn.Embedding(nb_responses, 2*n_dims)
    ...
    def forward(self, input_exercise, input_skill, input_r):
        ...
        _skill = _skill + _pos
        _exe = _exe + _pos
        input_encoder = torch.cat([_skill, _exe], dim=-1)
